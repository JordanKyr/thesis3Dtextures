...
public class HapticPlugin : MonoBehaviour
{
    ...
    [Foldout(title= "Player", order=2)]             //input gia methodo otan button click
    [Group("G5",1)]
    [Label(title ="Player")]
    [StackableField]
    ...
    {    localY=transform.localPosition.y;
        ...
        initial_device_rot=new GameObject("Temp"); //pairno arxiki rot tis siskeuis
    ...
    private void CameraUpdate()
    {
        ...
        masterCamera.transform.rotation = transform.rotation * inital_camera_rot.transform.rotation;     //SOSTOS KODIKAS
       masterCamera.transform.position = transform.position - distance * masterCamera.transform.forward;   //SOSTOS KODIKAS
        ...
        //AN KRATISO STATHERI THN KAMERA KAI VALO TO WORKSPACE NA THN AKOLOUTHISEI

        //transform.rotation=masterCamera.transform.rotation*initial_device_rot.transform.rotation;
        
        
        //transform.position= masterCamera.transform.position-distance * transform.forward;  //to rotation tha ginei me kentro thn kamera
  ...
  if (SceneLimit == false)                                  //an den exo SceneLimit metakinoime pantoy
        {
    ...
    if (FreezeTranslation.HasFlag(Axis.Z) || !GetDZZero(2))  //ΔΙΟΡΘΩΣΗ ΗΤΑΝ ΣΤΟ ΕΛΣΕ ΤΟΥ Υ ΚΑΙ ΑΠΕΝΕΡΓ. ΟΤΑΝ ΑΕΝΕΡΓ ΤΟ Υ
                {
                    stz = 0.0f;
                }
                else
        ...
        transform.parent.transform.Translate(stx, sty, stz);  //SOSTO KOYNAEI TON PARENT OPOTE KOUNIETAI KAI TO ACTOR
    ...
    public void UpdateTransfrom()
    {
       ...
       /* 
        Vector3 testV;
       float pX = rBody.position.x*-1.0f;
       float pY = rBody.position.y*-1.0f;
      float pZ = rBody.position.z*-1.0f;
        
        testV=new Vector3(pX,pY,pZ);
        
     deltaPos = targetPos -  testV; */
        ...
        if (transformDirect==true)
        {
           ...
           // targetPos.z=-targetPos.z;
            ...
            VisualizationMesh.transform.SetPositionAndRotation(newMatrix.ExtractPosition(), newMatrix.ExtractRotation());
           // VisualizationMesh.transform.SetPositionAndRotation(targetPos, newMatrix.ExtractRotation());
           // CollisionMesh.transform.position=targetPos;
...
#region Buttons
    ...
    public void UpdateButtonStatus()
    {
        ...
        if (LastButtons[0] == 0 && Buttons[0] == 1)  // STROFI
        {
        ...
        // Button 2
        if (LastButtons[1] == 0 && Buttons[1] == 1) //STROFI
        {
...
public class FieldPropertiesAttribute : UnityEngine.PropertyAttribute
{
...
public class DisplayOnlyAttribute : UnityEngine.PropertyAttribute
{
